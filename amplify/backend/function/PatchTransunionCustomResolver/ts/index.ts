...
import { patchTransunionInDB } from 'lib/queries/transunion.queries';
...
const resolvers: Record<string, any> = {
  Mutation: {
    patchTransunion: async (event: IResolverEvent) => {
      try {
        return await patchTransunionInDB(event.arguments.id, event.arguments.msg);
      } catch (err) {
