...
export class SettingsPureComponent {
  @ViewChild(FilledClosingAlertComponent) alert:
    | FilledClosingAlertComponent
    | undefined;

  @Output() logoutClick: EventEmitter<void> = new EventEmitter();
  @Output() changePasswordClick: EventEmitter<void> = new EventEmitter();
  @Output() deactivateClick: EventEmitter<IDeactivateAccount> =
    new EventEmitter();
  @Output() goToPageClick: EventEmitter<{ tab: number; view: ISettingsViews }> =
    new EventEmitter();
