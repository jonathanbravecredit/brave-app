...
const setup = () => {
  ...
  const creditReportMock = jasmine.createSpyObj('CreditreportService', ['getNegativeItems'], {
...
describe('NegativeAccountService', () => {
  ...
  describe('navigate method', () => {
    it('should call broadcastService.broadcast', () => {
      const spy = spyOn(broadcastMock, 'broadcast');
      service.navigate('/myroute');
      expect(spy).toHaveBeenCalledWith(EventKeys.NAVIGATION, '/myroute');
    });
  });
});
