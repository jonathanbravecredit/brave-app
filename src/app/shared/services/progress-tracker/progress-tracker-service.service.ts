...
import {
  ...
  ProgressTrackerStateModel,
} from "@store/progress-tracker";
...
export class ProgressTrackerService implements OnDestroy {
  ...
  initiativeSteps$: BehaviorSubject<ICircleProgressStep[]> =
    new BehaviorSubject<ICircleProgressStep[]>([]);
  ...
  createInitiativeSteps(initiative: Initiative) {
    ...
    this.initiativeSteps =
      initTaskLen > 1 ? tasks.map(mapTask) : subTasks.map(mapTask);
  }
  ...
  updateProgressTrackerState(progressTrackerData: Initiative) {
    this.store.dispatch(
      new ProgressTrackerActions.Add({ data: progressTrackerData })
    );
  }
  ...
  async getProgressTrackerData(): Promise<Initiative | null> {
    ...
    try {
      let res = await this.http
        .get<Initiative>(environment.api + "/initiatives", { headers })
        .toPromise();
  ...
  async updateProgressTrackerData(
    patchBody: InitiativePatchBody
    ...
    try {
      let res = await this.http
        .put<Initiative>(environment.api + "/initiatives", patchBody, {
          headers,
        })
        .toPromise(); //!change test url
  ...
  async postUserGoal(goalInfo: IGoalInfo) {
    ...
    return await this.http
      .post<Initiative>(environment.api + "/initiatives", patchBody, {
        headers,
      })
      .toPromise();
  }
