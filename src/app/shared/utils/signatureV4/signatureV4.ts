...
export class SigV4 {
  ...
  buildCanonicalRequest(method: string, path: any, queryParams: any, headers: any, payload: any) {
  ...
  buildCanonicalQueryString(queryParams: { [x: string]: string | number | boolean; hasOwnProperty?: any }) {
  ...
  buildCanonicalSignedHeaders(headers: { hasOwnProperty: (arg0: string) => any }) {
    let sortedKeys = [];
    for (let property in headers) {
      if (headers.hasOwnProperty(property)) {
  ...
  buildCredentialScope(datetime: string, region: string | null, service: string | null) {
    return datetime.substr(0, 8) + '/' + region + '/' + service + '/' + this.AWS4_REQUEST;
  }

  calculateSigningKey(secretKey: string, datetime: string, region: string | null, service: string | null) {
    return this.hmac(
  ...
  signRequest(request: { method: string; path: any; queryParams: any; headers: any; body: any }) {
